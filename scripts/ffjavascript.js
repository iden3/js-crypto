...
async function buildEngine(params) {
    ...
    const tm = await buildThreadManager(params.wasm);
...
async function buildBn128(plugins) {
...
async function buildBls12381(plugins) {
...
async function getCurveFromR(r, plugins) {
    let curve;
    if (eq(r, bn128r)) {
        curve = await buildBn128(plugins);
    } else if (eq(r, bls12381r)) {
        curve = await buildBls12381(plugins);
    } else {
...
async function getCurveFromQ(q, plugins) {
    let curve;
    if (eq(q, bn128q)) {
        curve = await buildBn128(plugins);
...
async function getCurveFromName(name, plugins) {
    let curve;
